...
describes what users get. Pick a format with `-o`, or let xueqiu choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
xueqiu <command> -o table   # aligned columns for reading
xueqiu <command> -o jsonl   # one JSON object per line, for piping
xueqiu <command> -o json    # a single JSON array
xueqiu <command> -o csv     # spreadsheet friendly
xueqiu <command> -o tsv     # tab-separated
xueqiu <command> -o url     # just the URL column
xueqiu <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
xueqiu <command> --fields id,title,url
```
...
```bash
xueqiu <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
xueqiu <command>            # a table, because this is a terminal
xueqiu <command> | wc -l    # JSONL, because this is a pipe
```
