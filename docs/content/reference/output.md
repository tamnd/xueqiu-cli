...
describes what users get. Pick a format with `-o`, or let xue choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
xue <command> -o table   # aligned columns for reading
xue <command> -o jsonl   # one JSON object per line, for piping
xue <command> -o json    # a single JSON array
xue <command> -o csv     # spreadsheet friendly
xue <command> -o tsv     # tab-separated
xue <command> -o url     # just the URL column
xue <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
xue <command> --fields id,title,url
```
...
```bash
xue <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
xue <command>            # a table, because this is a terminal
xue <command> | wc -l    # JSONL, because this is a pipe
```
