---
title: "Resource URIs"
description: "Use xue as a database/sql-style driver so a host program can address xueqiu as xueqiu:// URIs."
weight: 20
---

`xue` is a command line, but the `xueqiu` Go package is also a
...
host `xueqiu.com`. The standalone `xue` binary does not change.
...
answers both `xue page` on the command line and `ant get
