// Package cli assembles the xueqiu command tree from the xueqiu
...
// NewApp assembles the kit application from the xueqiu domain. The
...
//
// To add a command, declare it in xueqiu/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := xueqiu.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(xueqiu.Domain{}).Register(app)
